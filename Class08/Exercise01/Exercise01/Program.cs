using Exercise01.MyLibrary.Classes;
...
namespace Exercise01
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Jerry.FavoriteSong = Songs.Where(song => song.Title.StartsWith("B")).ToList();
            Console.WriteLine("Jerry Songs:");
            Jerry.GetFavSongs();
            Console.WriteLine("-----------------");

            Maria.FavoriteSong = Songs.Where(song => song.Length > 360).ToList();
            Console.WriteLine("Maria Songs:");
            Maria.GetFavSongs();
            Console.WriteLine("-----------------");

            Jane.FavoriteSong = Songs.Where(song => song.Genre == Genre.Rock).ToList();
            Console.WriteLine("Jane Songs:");
            Jane.GetFavSongs();
            Console.WriteLine("-----------------");

            Stefan.FavoriteSong = Songs.Where(song => song.Length < 180 && song.Genre == Genre.Hip_Hop).ToList();
            Console.WriteLine("Stefan Songs:");
            Stefan.GetFavSongs();
            Console.WriteLine("-----------------");

            foreach (var person in FansArray)
            {
                if(person.FavoriteSong.Count >= 4)
                {
